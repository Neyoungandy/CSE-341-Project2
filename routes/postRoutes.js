...
router.post(
    "/",
    authenticateJWT, //  Require authentication before creating a post
    [
...
router.put(
    "/:id",
    authenticateJWT, //  Require authentication before updating a post
    [
...
router.delete(
    "/:id",
    authenticateJWT, //  Require authentication before deleting a post
    [param("id").isMongoId().withMessage("Invalid Post ID")],
    (req, res, next) => {
        const errors = validationResult(req);
        if (!errors.isEmpty()) {
